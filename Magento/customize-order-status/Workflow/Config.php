...
class Config
{
     ...
     * @return array
     */
    public function getStateStatuses($state, $addLabels = true, $orderid='')
    {
	    $latest_status = '';
	    $order = [];
    	if($orderid != ''){
    		$objectManager = \Magento\Framework\App\ObjectManager::getInstance();
    		$orderinc = $objectManager->create('Magento\Sales\Model\Order')->load($orderid);
    		$order = $orderinc->getData();
    		$payment_method = $orderinc->getPayment()->getMethodInstance()->getTitle();
    		$latest_status = $this->getStatusLabel($order['status']);
    		$i = 0;
    		$states_item = '';
    		foreach($orderinc->getStatusHistoryCollection(true) as $_item){
    			if($i == 1){
    				$states_item = $_item->getStatusLabel();
    			}
    		    $i++;			
    		}
	    }
	    ...
	    if($orderid != ''){
    		...
    		if($latest_status == 'Card Confirmed'){
    			unset($statuses);
    			$statuses['accept'] = 'Accepted';
    			$statuses['reject'] = 'Rejected';
    		}
    		if($latest_status == 'Cash Confirmed'){
    			unset($statuses);
    			$statuses['accept'] = 'Accepted';
    			$statuses['reject'] = 'Rejected';
    		}
    		...
    		if($latest_status == 'On Hold'){
    			unset($statuses);
    			if($states_item == 'Yes'){
    				$statuses['in_kitchen'] = 'In Kitchen';
    			} else if($states_item == 'In Kitchen'){
    				$statuses['on_route'] = 'On Route';
    			} else if($states_item == 'On Route'){
    				if($payment_method == 'Credit Card (saved)'){
    					$statuses['delivered'] = 'Delivered';
    				}
    				if($payment_method == 'Cash On Delivery'){
    					$statuses['pending_payment'] = 'Cash Received';
    				}
    			} else if($states_item == 'Cash Received'){
    				$statuses['delivered'] = 'Delivered';
    			}
    		}
